...
interface OwnerDashboardProps {
  /** 'desktop' for multi-panel layout, 'mobile' for vertical scrollable cards */
  variant: 'desktop' | 'mobile';
}

export default function OwnerDashboard({ variant }: OwnerDashboardProps) {
  ...
  // Clear errors when villa changes
  useEffect(() => {
    setSectionErrors([]);
  }, [selectedVilla]);
      ...
      <div className="min-h-screen bg-[#0E0E0E] text-white font-sans pb-20">
        ...
        <header className="sticky top-0 z-20 bg-[#0E0E0E]/95 backdrop-blur border-b border-white/5 px-4 py-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-heritage-gold font-serif text-lg">Owner Portal</p>
        ...
        <div className="px-4 space-y-4">
          <ManagementOverview villaId={selectedVilla} onError={handleSectionError('management')} />
          <AssetTracking villaId={selectedVilla} onError={handleSectionError('assets')} />
          <SecurityAccess villaId={selectedVilla} onError={handleSectionError('security')} />
          <FinancialSummary villaId={selectedVilla} onError={handleSectionError('financial')} />
        </div>
      </div>
    );
  }

  // Desktop variant - multi-panel layout
  return (
    <div className="flex h-screen bg-[#0E0E0E] text-white overflow-hidden font-sans">
      ...
      <aside className="hidden lg:flex w-64 flex-shrink-0 border-r border-white/5 flex-col bg-[#0A0A0A]">
        ...
        <div className="p-6 border-b border-white/5">
          <p className="text-heritage-gold font-serif text-xl leading-none">Owner Portal</p>
          <p className="text-white/30 text-[10px] uppercase tracking-widest mt-1">Villa Management</p>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="sticky top-0 z-10 bg-[#0E0E0E]/90 backdrop-blur border-b border-white/5 px-6 py-4 flex items-center justify-between flex-shrink-0">
          <div>
            <h1 className="text-lg font-serif text-white">{currentVilla.name}</h1>
            <p className="text-white/30 text-[9px] uppercase tracking-widest mt-0.5">
              {currentVilla.location} · Owner Dashboard
            </p>
          </div>
        ...
        {/* Dashboard Content - Multi-panel layout */}
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="space-y-6"
            >
              {/* Top row: Management Overview full width */}
              <ManagementOverview villaId={selectedVilla} onError={handleSectionError('management')} />

              {/* Middle row: Asset Tracking + Security Access */}
              <div className="grid lg:grid-cols-2 gap-6">
                <AssetTracking villaId={selectedVilla} onError={handleSectionError('assets')} />
                <SecurityAccess villaId={selectedVilla} onError={handleSectionError('security')} />
              </div>

              {/* Bottom row: Financial Summary full width */}
              <FinancialSummary villaId={selectedVilla} onError={handleSectionError('financial')} />
            </motion.div>
          </AnimatePresence>
        </main>
